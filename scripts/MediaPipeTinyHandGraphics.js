...
export const handScale = 2;
export function updateHands(pinchPositionsSS, hands)
{
    ...
    {
        if (pinchPositionsSS[h] == undefined){
            break;
        }
        let pinchPositionSS = {x: pinchPositionsSS[h].x,
            y: pinchPositionsSS[h].y };
        ...
        if (hands[h].fingers.length > 0) 
        {
            ...
            for (let f = 0; f < 5; f++)
            {
                ...
                for (let j = 0; j < 4; j++)
                {
                    let joint = hand.fingers[f].joints[j];
                    let jointX = -joint.x + pinchPositionSS.x;
                    let jointY = -joint.y + pinchPositionSS.y;
                    let jointZ = joint.z;
                    finger.push({
                        x: jointX,
                        y: jointY,
